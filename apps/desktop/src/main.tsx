...
function App() {
  ...
  const [rendered, setRendered] = React.useState<Array<{fileName: string; url: string}>>([]);
  ...
  const renderExport = async () => {
    ...
    try {
      ...
      const result = await response.json();
      setRendered(result.outputs.map((output: {fileName: string; url: string}) => ({ ...output, url: `http://127.0.0.1:8000${output.url}` })));
